...
基于 Edge + Ollama 的免费开源翻译扩展。为方便使用本地模型阅读网页而做：连接一次，一键翻译，滚动时继续，随时恢复原文。
...
升级后需重新运行 `install.cmd` 以启动 0.9.1 本机服务，再到 `edge://extensions` 重新加载扩展并刷新网页。旧的模型和术语会保留；外部 API Key 不参与设置导出。
